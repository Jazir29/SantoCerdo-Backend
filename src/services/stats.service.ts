...
export async function getDashboard(params: {
  ...
  const { range, status, customerType, startDate, endDate } = params;
  ...
  const [
    ...
    pool.query(`SELECT SUM(o.total_amount)   as total ${revWhere}`, revP),
    pool.query(`SELECT SUM(o.discount_amount) as total ${revWhere}`, revP),
    pool.query(`SELECT COUNT(*) as count ${allWhere}`, allP),
    pool.query(`SELECT COUNT(DISTINCT o.customer_id) as count ${allWhere}`, allP),
    pool.query(`SELECT AVG(o.total_amount) as avg ${revWhere}`, revP),
    pool.query('SELECT COUNT(*) as count FROM products WHERE stock <= min_stock AND deleted_at IS NULL'),
    pool.query(`
      SELECT p.name,
    ...
    pool.query(`
      ...
      GROUP BY c.type
    `, [...dateFilt.params, ...revStatusFilt.params]),
    pool.query(`
      SELECT DATE(o.created_at) as date,
    ...
    pool.query(`
      ...
      GROUP BY o.status
    `, [...dateFilt.params, ...typeFilt.params]),
  ]) as any[];
